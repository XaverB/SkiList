...
class skiplist {
 ...
 public:
  skiplist() : level(0), head(nullptr) { initialize_srand_if_necessary(); };
  ~skiplist(){};
  ...
  value_type get(key_type key) {
    ...
    nodeptr x = head;
    if (x->get_key() == key) return x->get_value();

    for (int i = level; i >= 1; i--) {
      while (x->get_forward(i) != nullptr &&
             x->get_forward(i)->get_key() < key) {
        x = x->get_forward(i);
      }
    }
    x = x->get_forward(1);
  ...
  void insert(key_type key, value_type value) {
    ...
    nodeptr x = head;
    if (x == nullptr) {
      head = std::make_shared<node>(key, value, ++level);
      return;
    }
  ...
  void remove(key_type key) {
    ...
    if (x != nullptr && x->get_key() == key) {
      for (int i = 1; i <= level; i++) {
        if (update[i] != nullptr && update[i]->get_forward(i) != x) {
          break;
        }
      ...
      while (level > 1
        && head->get_forward(level) == nullptr) {
        level--;
      }
    }
    delete[] update;
